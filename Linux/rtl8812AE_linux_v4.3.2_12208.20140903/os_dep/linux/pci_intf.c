...
static s32	rtw_set_pci_cache_line_size(struct pci_dev *pdev, PPCI_COMMON_CONFIG pPciConfig, u8 CacheLineSizeToSet)
{
...
static s32 rtw_pci_parse_configuration(struct pci_dev *pdev, struct dvobj_priv *pdvobjpriv, u8 *pucBuffer)
{
	struct pci_priv	*pcipriv = &(pdvobjpriv->pcipriv);
	...
	do{
		...
		//3 Fill Cacheline 
		ret = rtw_set_pci_cache_line_size(pdev, pPciConfig, 8);
		if(ret != _SUCCESS)
		{
...
#endif // !#ifdef CONFIG_SUSPEND_REFINE	
static int rtw_pci_resume(struct pci_dev *pdev)
{
	struct dvobj_priv *dvobj = pci_get_drvdata(pdev);
	_adapter *padapter = dvobj->if1;
