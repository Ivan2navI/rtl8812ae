	...
	)
{
	...
	{
		// VID, DID 
		pHalData->EEPROMVID = EF2Byte( *(u16 *)&PROMContent[EEPROM_VID_8812AE] );
		pHalData->EEPROMDID = EF2Byte( *(u16 *)&PROMContent[EEPROM_DID_8812AE] );
		pHalData->EEPROMSVID = EF2Byte( *(u16 *)&PROMContent[EEPROM_SVID_8812AE] );
		pHalData->EEPROMSMID = EF2Byte( *(u16 *)&PROMContent[EEPROM_SMID_8812AE] );
	...
	)
{
	...
	else
	{
		...
		if(IS_HARDWARE_TYPE_8812E(pAdapter))
			_rtw_memcpy(pEEPROM->mac_addr, &PROMContent[EEPROM_MAC_ADDR_8812AE], ETH_ALEN);
		else if(IS_HARDWARE_TYPE_8821E(pAdapter))
			_rtw_memcpy(pEEPROM->mac_addr, &PROMContent[EEPROM_MAC_ADDR_8821AE], ETH_ALEN);
	}
	...
	)
{
	...
	if(!AutoloadFail)
	{
		...
		DBG_871X("Hal_ReadEfusePCIeCap8812AE(): PCIeCap = %#x\n", PCIeCap);
	...
	DBG_871X("Hal_ReadEfusePCIeCap8812AE(): AspmOscSupport = %d\n", AspmOscSupport);
}
...
)
{
	...
	DBG_871X("hal_CustomizedBehavior_8812E(): RT Customized ID: 0x%02X\n", pEEPROM->CustomerID);
	...
	)
{
	...
	hal_InitPGData_8812E(Adapter, pEEPROM->efuse_eeprom_data);
	...
	hal_ReadIDs_8812E(Adapter, pEEPROM->efuse_eeprom_data, pEEPROM->bautoload_fail_flag);
	hal_ReadMACAddress_8812E(Adapter, pEEPROM->efuse_eeprom_data, pEEPROM->bautoload_fail_flag);	
	...
	Hal_ReadEfusePCIeCap8812AE(Adapter, pEEPROM->efuse_eeprom_data, pEEPROM->bautoload_fail_flag);
	...
	Hal_ReadAntennaDiversity8821A(Adapter, pEEPROM->efuse_eeprom_data, pEEPROM->bautoload_fail_flag);
	Hal_ReadPAType_8821A(Adapter, pEEPROM->efuse_eeprom_data, pEEPROM->bautoload_fail_flag);
	//Hal_EfuseParseRateIndicationOption8821A(Adapter, pEEPROM->efuse_eeprom_data, pEEPROM->bautoload_fail_flag);
	...
	)
{
	...
	//DBG_871X("InitAdapterVariablesByPROM_8812E()!!\n");
	...
	//Hal_EfuseParseRateIndicationOption8812A(Adapter, pEEPROM->efuse_eeprom_data, pEEPROM->bautoload_fail_flag);
	hal_CustomizeByCustomerID_8812E(Adapter);
}
...
static void ReadAdapterInfo8812AE(PADAPTER Adapter)
{
	...
	//DBG_871X("====> ReadAdapterInfo8812E\n");
	
	// For debug test now!!!!!
	PHY_RFShadowRefresh(Adapter);

	//pHalData->VersionID = ReadChipVersion8812A(Adapter);
...
)
{
	...
	DBG_871X("=======>InitMAC_8812()\n");

	_InitPowerOn_8812AE(Adapter);
...
_func_enter_;
	...
	//
#if (HAL_MAC_ENABLE == 1)
	DBG_871X("8812 MAC Config Start!\n");
	rtStatus = PHY_MACConfig8812(Adapter);
	if (rtStatus != _SUCCESS)
	{
		DBG_871X("8812 MAC Config failed\n");
		return rtStatus;
	}
	DBG_871X("8812 MAC Config Finished!\n");
...
/*{
	...
	rtStatus = rtl8812a_iol_efuse_patch(Adapter);
	#endif
